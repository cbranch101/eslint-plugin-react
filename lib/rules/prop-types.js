  ...
  create: Components.detect((context, components, utils) => {
    ...
    */
    ...
    function isPropTypesUsage(node) {
      // only return usages for bottom level member expression
      ...
      const isInConstructor = inConstructor();
      ...
      // using props.foo in a class component, anywhere other than ComponentWillReceiveProps or a constructor
      if (isInClassComponent && !isInConstructor && !inComponentWillReceiveProps() && isDirectProp) {
        return false
      }
    ...
    /**
     * get the allNames array for a given property
     * @param {ASTNode} property a property node.
     */
    function getAllNamesForMemberExpression(expression, parentNames) {
      parentNames = parentNames || []
      const propertyName = getPropertyName(expression)
      const nameToAdd = propertyName ||
        (expression.type === 'ThisExpression' && 'this') ||
        (expression.name)
      const childNames = nameToAdd ?
        parentNames.concat(nameToAdd)
        : parentNames
      if (expression.object) {
        return getAllNamesForMemberExpression(expression.object, parentNames).concat(childNames)
      }
      return childNames
    }

    function traverseMemberExpression(expression, func, parentNames) {
      parentNames = parentNames || []
      const propertyName = getPropertyName(expression);
      if (propertyName) {
        const childNames = parentNames.concat(propertyName);
        func(propertyName, childNames, expression.property)
        if (expression.parent.type === 'MemberExpression') {
          return parentNames.concat(traverseMemberExpression(expression.parent, func, childNames));
        }
      }
      return parentNames
    }
     ...
     * @param {ASTNode} properties The AST node being marked.
     */
    function markPropTypesAsUsed(node) {
      ...
      switch (node.type) {
        case 'MemberExpression':
          ...
          const nodeToTraverse = isThisProps ? node.parent : node;
          const directlyOffOfProps = (nodeToTraverse.object.name === 'props' || nodeToTraverse.object.name === 'nextProps')
          if (!directlyOffOfProps && !isThisProps) {
            const lookupName = nodeToTraverse.object.name
            parentUsage = usedPropTypes.find(
              propType => propType.name === lookupName
            )
            if (!parentUsage) {
              break;
            }
            parentNames = parentUsage.allNames
          }

          traverseMemberExpression(nodeToTraverse, (propertyName, currentNames, currentNode) => {
        ...
        case 'VariableDeclarator':
          ...
          const isInClassComponent = (utils.getParentES6Component() || utils.getParentES5Component())
          const initNames = getAllNamesForMemberExpression(node.init)
          const isInStatelessComponent = !!utils.getParentStatelessComponent() & !isInClassComponent
          const thisPropsDestructuring = (
            isInClassComponent &&
            initNames[0] === 'this' && PROPS_REGEX.test(initNames[1])
          );
          // let {firstname} = props
          const directDestructuring =
            PROPS_REGEX.test(initNames[0]) &&
            (isInStatelessComponent || inConstructor() || inComponentWillReceiveProps())
          ;

          if (thisPropsDestructuring) {
            allNames = initNames.slice(2)
          }

          if (directDestructuring) {
            allNames = initNames.slice(1)
          }
          ...
          for (let i = 0, j = node.id.properties.length; i < j; i++) {
            ...
            properties = thisDestructuring ? node.id.properties[i].value.properties : node.id.properties
            if (!thisDestructuring || !directDestructuring || !thisPropsDestructuring) {
              parentUsage = usedPropTypes.find(
                propType => propType.name === initNames[0]
              )
            ...
            if (!thisDestructuring && !directDestructuring && !parentUsage && !thisPropsDestructuring) {
              continue;
            }
            type = 'destructuring';
            break;
      ...
      switch (type) {
        case 'destructuring':
          traverseProperties(properties, (key, value, property, allPropertyNames) => {
            ...
            allNames = allNames || []
            if (propName) {
              const allParentNames = parentUsage ? parentUsage.allNames.concat(allNames) : allNames
              usedPropTypes.push({
                name: isRenamedProperty ? propValueName : propName,
                allNames: allParentNames.concat(allPropertyNames),
                node: property
              });
            }
          })
    ...
    return {
      ...
      VariableDeclarator: function(node) {
        // const isInClassComponent = utils.getParentES6Component() || utils.getParentES5Component();
        // const inComponent = utils.getParentStatelessComponent() || inConstructor()
        // || inComponentWillReceiveProps() || isInClassComponent;
        // const destructuringInComponent =
        //    &&
        //   inComponent;
      ...
      'Program:exit': function() {
        ...
        for (const component in list) {
          ...
          // console.log(list[component].usedPropTypes)
