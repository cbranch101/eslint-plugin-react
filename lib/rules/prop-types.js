    ...
    */
    ...
    function traverseProperties(properties, fn, keys) {
      ...
      iterateProperties(properties, (key, value, property) => {
        const keyValue = getKeyValue(property)
        keys.push(keyValue)
        fn(keyValue, value, property, keys)
        if (value.properties) {
          traverseProperties(value.properties, fn, keys)
        }
      })
    }
    ...
    function markPropTypesAsUsed(node, parentNames) {
      ...
      switch (node.type) {
        ...
        case 'VariableDeclarator':
          for (let i = 0, j = node.id.properties.length; i < j; i++) {
            ...
            if (!directDestructuring || !thisDestructuring) {
              initName = node.init.name
            }
            parentType = initName && usedPropTypes.find(
              propType => propType.name === initName
            )




            if (!directDestructuring && !thisDestructuring && !parentType) {
              continue
            }
      ...
      switch (type) {
        case 'direct':
          ...
          break;
        case 'destructuring':
          traverseProperties(properties, (propName, value, property, allPropertyNames) => {

            if (hasSpreadOperator(property) || property.computed) {
              return;
            }
            allNames = allNames || [];
            const allParentNames = parentType ? parentType.allNames.concat(allNames) : allNames
            if (propName) {
              usedPropTypes.push({
                parentName: initName,
                name: propName,
