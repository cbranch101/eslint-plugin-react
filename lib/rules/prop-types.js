  ...
  create: Components.detect((context, components, utils) => {
    ...
    */
    ...
    function parentIsDestructured(node) {
      return node.parent.type === 'VariableDeclarator' && isDestucturedVariableDeclaration(node.parent);
    }
    ...
    return {
      ...
      VariableDeclarator: function(node) {
        if (isDestucturedVariableDeclaration(node) && node.init.type !== 'MemberExpression') {
          markPropTypesAsUsed(node);
        }
      },
